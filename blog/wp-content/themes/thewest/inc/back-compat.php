...
function theme_upgrade_notice() {
    $message = sprintf( __( 'This theme requires at least WordPress version 4.2. You are running version %s. Please upgrade and try again.', 'thewest' ), $GLOBALS['wp_version'] );
...
function theme_customize() {
    wp_die( sprintf( __( 'This theme requires at least WordPress version 4.2. You are running version %s. Please upgrade and try again.', 'thewest' ), $GLOBALS['wp_version'] ), '', array(
        'back_link' => true,
    ) );
}
...
function theme_preview() {
    if ( isset( $_GET['preview'] ) ) {
        wp_die( sprintf( __( 'This theme requires at least WordPress version 4.2. You are running version %s. Please upgrade and try again.', 'thewest' ), $GLOBALS['wp_version'] ) );
    }
}
